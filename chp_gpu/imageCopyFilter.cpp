...
/// \brief Main function
///
/// Takes one or two arguments (depends on the kernel, read copyimage.cl and the README file for more info)
...
/* print information on openCL version */
  ...
  /*début modification */
  status = clSetKernelArg(kernel, argIndex++, sizeof(cl_int), &n);
  if (kernel_name.compare("gauss")==0){
    float sigma=atof(argv[2]);
    float * exponentials = (float*) malloc((n+1)*sizeof(float));
    float msigm= -0.5*pow(sigma,-2);
    for (int k=0; k<=  n; k++) {
      exponentials[k]= exp(k*k*msigm);
    }
    cl_mem cl_exponentials=clCreateBuffer(context,CL_MEM_READ_ONLY| CL_MEM_COPY_HOST_PTR, (n+1)*sizeof(float), exponentials, NULL);
    status = clSetKernelArg(kernel, argIndex++, sizeof(cl_mem), (void*) &cl_exponentials);
  ...
  /* début modification */
  /*if (kernel_name.compare("gauss")==0){
    status = clReleaseMemObject(cl_exponentials); } */
  /* fin modification */
